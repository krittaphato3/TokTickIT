...
- **Implemented (issue #41):** §5.3 shipped on `#/tickets/:number` — two-tab shell (Public Comments, Attachments) replacing the Lab 2 mock tabs; live thread with author + role badge + timestamp + escaped bodies; composer textarea with hint copy, live 2000-char counter, disabled Post when empty/whitespace/over-limit, busy/success/failure feedback; pale-green banner + flagged chip for the appears-resolved indication; signal button gated to the ticket's requester while status ∉ {Resolved, Closed, Cancelled} and disabled after one indication with the "Marked as appearing resolved ✓" caption; confirm modal per the copy above (signal posts a flagged comment; status unchanged). The Not Found ownership display above is rendered by the existing detail error state.

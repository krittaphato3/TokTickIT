...
| T-ADM-01 | API | FR-11, BR-10 / AC-15 | Admin user list with search by name/email | 200 with exactly matching users | `server/tests/lab-03/users-admin.api.test.ts` | Implemented (id-ascending list with `{data, meta.totalItems}`, no `passwordHash` in any row; case-insensitive substring search over name AND email; blank search = no filter) |

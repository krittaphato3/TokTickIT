...
- **Plan policy:** Plan written before implementation. `Final` column records planned outcome `Pass-planned` for every row prior to execution. Auth-foundation rows implemented in `server/tests/lab-03/auth.api.test.ts` (T-AUTH-01..05, T-PWD-01, T-SESS-01, T-GATE-01, T-MIG-01) record `Implemented` below; all other rows stay `Pass-planned` (staff/admin issues own them).

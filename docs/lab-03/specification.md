...
- **FR-10 — Problem-appears-resolved indication.** A Requester on an owned ticket records a one-way "problem appears resolved" signal as a specialized public comment via `POST /api/tickets/:ticketNumber/comments` with `{appearsResolved:true}` hint (api-spec §7.2). The comment joins the append-only thread, sets the `appearsResolvedAt` timestamp, does not change status, has no separate route, is visible to staff as triage input, and is rate-limited to one active signal per ticket until staff changes status.
...
- **BR-05 — Requester-indication vs formal resolve.** The problem-appears-resolved signal is a boolean plus timestamp on the ticket, not a status value, recorded as a specialized public comment via `POST /api/tickets/:ticketNumber/comments` with `{appearsResolved:true}` hint (no separate route). Only IT Staff through the BR-15 matrix sets Resolved or Closed, except a Requester may transition Closed->Reopened (and Resolved->Reopened where allowed) via PATCH per BR-15; all other Requester status writes return 403/409. The `appearsResolved` signal does not change status. Staff sees the signal as "Requester indicates resolved" input to triage.
...
- **PublicComment model (§7.3 concretized):** `id`, `ticketId` (cascade), `authorId` → `User` (restrict), `body` VarChar(2000), `appearsResolved` Boolean default false, `createdAt`; index `(ticketId, createdAt asc)`. Append-only — no update/delete code paths exist. One migration: `20260914143757_lab3_public_comments`.
- **InternalNote model (shipped with the staff-detail issue, migration `20260916120000_lab3_staff_detail_ops`):** `id`, `ticketId` (cascade), `authorId` → `User` (restrict), `body` VarChar(2000), `createdAt`; index `(ticketId, createdAt asc)`. Append-only. Requester-facing masking is enforced in the route handlers with the masked 404 (api-spec §1.5), byte-identical for absent tickets.
- **Comment surface (§12, requester rows):** `GET|POST /api/tickets/:ticketNumber/comments` registered on the tickets router behind the global session/CSRF gates. Requester scope resolves ownership through the same `resolveSessionRequester` email-link path as the ticket endpoints; absent and foreign tickets are the identical masked 404 (BR-11). Staff/admin may comment on any ticket via the requester routes; the §7 staff aliases (`GET|POST /api/staff/tickets/:number/comments`) ship with the staff-detail issue, and staff status transitions append their BR-15 audit system comment to the same public thread.
...
- Auth: `POST /api/auth/login`, `POST /api/auth/logout`, `GET /api/auth/me`, `POST /api/auth/change-password` (mandatory and voluntary modes), `POST /api/auth/forgot-password` (credential-verified reset — no email, see AD-04).
...
- Comments: `GET|POST /api/tickets/:ticketNumber/comments` (public, including problem-appears-resolved signal via `POST /api/tickets/:ticketNumber/comments` with `{appearsResolved:true}` hint per api-spec §7.2 — no dedicated appears-resolved route); `GET|POST /api/tickets/:ticketNumber/notes` (internal, masked 404 for requesters).
...
| AD-04 | "Forgot password?" is implemented as a credential-verified self-service reset (`POST /api/auth/forgot-password`: email + current/initial password + new password in one request). No email is sent, no reset token exists, unknown-email/wrong-password/inactive all return the same safe 401, and a successful reset revokes all of the user's sessions. Email-based reset stays excluded per the lab sheet. | The approved UI mockup (`Authentication-Forgot-Mockup.html`) requires a working "Forgot password?" entry point, but the stakeholder sheet excludes email invitations and password-reset email. The simplest compliant interpretation keeps recovery self-service via credential proof — zero new infrastructure, no token/email surface to secure, full BR-16 masking preserved — and users who lost their password contact an administrator, exactly as the safe login copy implies. | Accepted |
...
| AD-06 | IT Priority is null (Unset) while unassigned; on first claim (unassigned→assigned) it copies Requested Priority, then independently editable by IT Staff only. | Prevents system-set IT priority before ownership; satisfies 'initially copies' at ownership time. | Accepted |

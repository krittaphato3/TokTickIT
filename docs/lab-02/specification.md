...
- **BR-20 — IT Priority and Owner Display Rules.** `itPriority` is a nullable display field set by IT staff tooling, not by the requester; a null `itPriority` renders as neutral "Unset". `ownerName` defaults to the creating Development Requester's `name` at `POST /api/tickets` time and is returned on create, list, and detail; it may later be reassigned by IT Staff tooling (out of scope for Lab 2) — a null `ownerName` renders as muted "Unassigned" for legacy/seed rows. The `itPriority` filter matches only tickets whose stored `itPriority` equals the given value — it never falls back to matching the requested priority.
